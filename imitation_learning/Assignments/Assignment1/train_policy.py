...
def train_discrete(model, iterator, opt, args):
    ...
    for i_batch, batch in enumerate(iterator):
        ...
        # compute cross_entropy loss with and without weights
        if args.weighted_loss == True:
            criterion = nn.CrossEntropyLoss(torch.from_numpy(1 / args.class_dist).to(DEVICE))
        else:
...
def get_class_distribution(training_dataset, args):
    # iterate over all samples instead of batch
    iterator = DataLoader(training_dataset, batch_size=1, shuffle=True, num_workers=1)
...
def main(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        val_acc = test_discrete(driving_policy, validation_iterator, opt, args)

        # If the accuracy on the validation set is a new high then save the network weights 
        if val_acc > best_val_accuracy:
            best_val_accuracy = val_acc
            print(f"Saving weights at:{args.weights_out_file}")
