...
def find_neigbours(node_list, new_node, radius, rrt_dubins):
   """
   input: node_list, radius
   output: neighbours
   """
   neighbours = []
...
def rrt_star_planner(rrt_dubins, display_map=False):
   ...
   while i < rrt_dubins.max_iter:
      ...
      new_node = rrt_dubins.propogate(nearest_node, rnd_node)
      ...
      if rrt_dubins.check_collision(new_node):
         ...
         # radius
         node_number = len(rrt_dubins.node_list) + 1
         radius = np.minimum(gamma*(np.log(node_number) / (np.log(node_number)))**(1/3), min_rad)
         
         # find the neighbours for the new node
         neighbors = find_neigbours(rrt_dubins.node_list, new_node, radius, rrt_dubins)
         
         x_min = nearest_node
         min_cost = rrt_dubins.calc_new_cost(new_node, nearest_node)

         # optimize cost for each node
         for node in neighbors:
            cost = rrt_dubins.calc_new_cost(new_node, node)
            temp_node = new_node
            temp_node.parent = node
            if (rrt_dubins.check_collision(temp_node) and cost < min_cost):
               x_min = node
               min_cost = cost
               
         rrt_dubins.node_list[-1].parent = x_min
